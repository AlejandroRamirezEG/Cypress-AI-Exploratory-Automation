...
- `WCAG_AUTH_TOKEN` + `WCAG_AUTH_TOKEN_KEY` — for pages that require auth but have no reachable login redirect (e.g. a hidden report route set as `BASE_URL`): the token is written into `localStorage[WCAG_AUTH_TOKEN_KEY]` via `onBeforeLoad` on the initial `cy.visit`, before the app boots. Applies to both modes.
- `WCAG_LOGIN_URL` — interactive-mode-only fallback when no static token is set: visits this login page first so the tester signs in through the real UI, then uses the control bar's 🔗 field to jump to `BASE_URL`.
...
`injectScanControls` also calls `installNavigationGuard(win)` on every invocation (every cycle and every re-injection). Cypress only controls the single iframe it renders the AUT in — a `window.open()` call or an `<a target="_top"|"_parent"|"_blank">` click escapes that iframe into the Cypress Runner's own top-level window, which then shows the Runner's own "you seem to have gotten lost" 404 (a relative `href` resolves against the Runner's origin, not the AUT's, once it's navigating the Runner's own top frame). The guard stubs `window.open` and adds a capture-phase `click` listener that intercepts target-breaking anchors, rewriting both to `navigateWithinFrame(rootWin, url)` — the same helper the 🔗 field uses, so a hash-only destination (same origin+path, different hash) forces a real `win.location.reload()` instead of a live in-app route swap.

`installNavigationGuard(win, rootWin = win)` takes a second param specifically for apps that embed a same-origin iframe running their own instance of the app (e.g. an "embed mode" widget) — `rootWin` is captured once at the top-level call and threaded through the recursive same-origin-iframe walk unchanged, so every escape attempt from any nesting depth lands on the AUT's own outermost document, not on whichever frame the click fired from. This matters because native `target="_top"` means "the top of the *entire* frame tree" — inside Cypress that's the Runner's window (two levels up from a nested app iframe), not the AUT's root (one level up), which is what `_top` would resolve to in a real, non-Cypress browser tab. Without `rootWin`, a naive same-frame rewrite would leave the click's destination rendering *inside* the nested iframe — which looked like the app re-rendering its own shell in place (duplicate header/logo: one from the AUT's outer document, one from the freshly-booted instance inside the iframe) rather than actually navigating away from it. Guarded by `win.__wcag_nav_guard_installed__` so re-running it against an unchanged document (a same-page SPA route change) is a no-op, while a genuinely fresh document (full reload, or a same-origin iframe just discovered) gets it installed. Covers same-origin iframes present at install time via an initial recursive walk, and a `MutationObserver` on `win.document` (`win.__wcag_nav_observer__`) covers any inserted afterwards — some report viewers (e.g. this app's legacy `path3-crm` embed) lazy-load their iframe well after the outer page's guard already ran, and a same-page insertion never replaces the outer document, so `barMissing` never fires to trigger rediscovery via `injectScanControls`. The observer guards a new `<iframe>` immediately (its `contentWindow` exists as `about:blank` the instant it's inserted) and again on its `load` event, since a same-origin navigation swaps in a new `Window` object the guard flag doesn't carry over to. Cross-origin iframes can't be reached (`SecurityError`) and are skipped either way — a script-driven `window.top.location = …`/`window.parent.location = …` assignment (no click, no `window.open`) is also not interceptable, since `top`/`parent` are spec-unforgeable properties that can't be redefined; the 🔗 field's direct URL entry remains the reliable path for pages reachable only that way.

Control bar buttons (left to right): **🔍 Scan**, **👁 Focus**, **🔗** (go to URL), **↑/↓** (move), **⎯** (minimize to pill), **✕** (end session). The **👁 Focus** toggle injects `<style id="__wcag_focus_style__">` into the AUT (`*:focus` outline 4px #ff007f, excluding `#__wcag_controls__` and `#__wcag_pill__`) so the designer can walk the tab order visually between scans. Active state is read from the DOM on bar re-injection so it survives across scan cycles. When focus escapes the AUT (`window.blur`), the bar turns amber (`#b45309`) and the message warns the tester; `window.focus` restores both. The **🔗** toggle reveals an inline `<input>` + Go button; submitting navigates via `win.location.href` (plain client-side navigation, not a new `win.__wcag_action__` value) — it exists for pages not reachable through the app's own nav (e.g. a hidden report route). This relies on same-origin navigation so `localStorage` (and any seeded auth token) survives the jump; it works because the existing bar-missing re-injection check (see below) restores the bar afterward exactly as it does after a login redirect. When the typed URL differs only by hash from the current location, the field forces a `win.location.reload()` right after setting the hash — a hash-only change is same-document per spec (no reload), so route guards relying on state set up during a fresh boot (auth checks, feature flags) would otherwise see live in-app state instead of a cold-boot state, and can reject the route even though the same URL works fine from a real address bar. Do not add new `win.__wcag_action__` values — extend the bar with toggles that only mutate DOM/styles/navigation.
